...
2 режима работы:
	1) Главное окно:
		Заполнение документа "Контрольный Список Измерений" и его определение в соответствующую папкам
	2) Split Scan окно:
		Обработка единого PDF файла отсканированных замеров и распределение каждого замера по папкам
...
Требования к каждому режиму:
	...
	Режим 2):
		...
		- Тексковый файл ".txt" с упорядоченным списком идентификационных названий (типа CWT.MP21-77-10.01.00)
		  отсканированных замеров. По умолчанию этот список создаётся и пополняется при каждом успешном исходе
		  1) режима работы.
...
Сравнение 2) режима работы без и с программой:
	Без программы:
		...
		2) Одностраничный "PDF" слить с соответствующим файлом "Контрольный Список Измерений" сгенерированным в режиме 1).
	...
	С программой:
		...
		2) Выбрать соответствующий тексковый файл ".txt" с упорядоченным
		   списком идентификационных названий (типа CWT.MP21-77-10.01.00)
...
Преимущества работы с программой:
	...
	- Названия обработанных документов сохраняются в текстовый список в папке "repo/"
	- Режим 2) распределяет сканы сам.
...
Возможные проблемы:
	...
	- Может показаться, что программа зависла. Она блокируется когда генерирует файлы и разблокируется по окончанию.
